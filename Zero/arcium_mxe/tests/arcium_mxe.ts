...
describe("ArciumMxe", () => {
  ...
  it("Classifies email score!", async () => {
    const owner = readKpJson(`${os.homedir()}/.config/solana/id.json`);

    console.log("Initializing classify email computation definition");
    const initCESig = await initClassifyEmailCompDef(
      program,
      owner,
      false,
      false
    ...
    console.log(
      "Classify email computation definition initialized with signature",
      initCESig
    );
    ...
    // Simulate an email score from LLM (e.g., 85 = good quality email)
    const emailScore = BigInt(85);
    const plaintext = [emailScore];
    ...
    console.log("Submitting encrypted email score:", emailScore.toString());
    ...
    const queueSig = await program.methods
      ...
      .accountsPartial({
        ...
        compDefAccount: getCompDefAccAddress(
          program.programId,
          Buffer.from(getCompDefAccOffset("classify_email")).readUInt32LE()
        ),
      })
      .rpc({ skipPreflight: true, commitment: "confirmed" });
    console.log("Queue sig is ", queueSig);
    ...
    const decryptedScore = cipher.decrypt([scoreEvent.score], scoreEvent.nonce)[0];
    console.log("Decrypted email score:", decryptedScore.toString());
    
    // Verify the score matches what we sent
    expect(decryptedScore).to.equal(emailScore);
    
    // Decision logic: score >= 70 means RELEASE, else WITHHOLD
    const decision = decryptedScore >= BigInt(70) ? "RELEASE" : "WITHHOLD";
    console.log(`Email decision: ${decision} (score: ${decryptedScore})`);
    expect(decision).to.equal("RELEASE");
  });

  async function initClassifyEmailCompDef(
    ...
    const offset = getCompDefAccOffset("classify_email");
    ...
    console.log("Classify email comp def pda is ", compDefPDA);

    const sig = await program.methods
      .initClassifyEmailCompDef()
      .accounts({
    ...
    console.log("Init classify email computation definition transaction", sig);

    if (uploadRawCircuit) {
      const rawCircuit = fs.readFileSync("build/classify_email.arcis");
